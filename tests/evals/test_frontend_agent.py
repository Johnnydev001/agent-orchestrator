...
test_frontend_agent = os.path.join(current_dir, "../datasets/frontend_intents.json")

dataset = EvaluationDataset()
dataset.add_goldens_from_json_file(test_frontend_agent)
...
def test_frontend_agent(golden: Golden):
    ...
    print("Test frontend agent result against golden expected output\n")
    assert_test(golden=golden, metrics=[TaskCompletionMetric()])

    print("Test frontend agent result\n")
    assert result["last_agent"] == "router_agent"
    assert result["next_agent"] == "frontend_agent"
